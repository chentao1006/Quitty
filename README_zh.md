...
由于 macOS 的安全机制限制，以及每个应用可能采用不同的窗口管理和技术实现，Quitty（静隐）无法保证对所有应用都能完全实现自动退出功能。我们始终遵循保守原则，尽量避免误退出正在使用的应用，但仍无法做到 100% 避免误操作。如果在使用过程中出现数据丢失或软件损坏，Quitty 概不负责，敬请谅解。如遇到无法退出或误退出的情况，欢迎提交 issue（[点击这里提交](https://github.com/chentao1006/quitty/issues)），我会在有空时尽力修复。感谢您的理解与支持！
