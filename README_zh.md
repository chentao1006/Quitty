...
5.  如果窗口总数为零（不包括抽屉和表单窗口），它将通过对比**已学习的幽灵窗口列表**和**自适应灵敏度乘数**进行二次确认。
...
- **绝不收集任何个人身份信息 (PII)**。
- 您可以随时在 `设置 > 通用` 中取消勾选“分享匿名使用数据”来**退出追踪**。

## 📝 注意事项

由于 macOS 的安全机制限制，以及每个应用可能采用不同的窗口管理和技术实现，静隐 无法保证对所有应用都能完全实现自动退出功能。我们始终遵循保守原则，尽量避免误退出正在使用的应用，但仍无法做到 100% 避免误操作。如遇到无法退出或误退出的情况，欢迎提交 issue（[点击这里提交](https://github.com/chentao1006/quitty/issues)），我会在有空时尽力修复。感谢您的理解与支持！
